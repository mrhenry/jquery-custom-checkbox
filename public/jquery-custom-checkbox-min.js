...
*/
(function(b){function d(a){var c=b(a);this.element=a;this.name=c.attr("name");this.escaped_name=this.name.replace("[","\\[").replace("]","\\]");this.$label=c.siblings("label[for="+c.attr("id")+"]");this.$fake=b('<span class="fake-checkbox"></span>');this.$groupling_labels=this.findGrouplingLabels();this.init()}d.prototype.init=function(){var a=this,c=b(a.element);c.hide();a.$label.addClass("custom-checkbox").append(a.$fake);a.setState();c.on("change",function(){a.setState()})};d.prototype.setState=
